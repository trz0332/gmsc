...
def jxconfig():
    ...
    for i in config['scrept']:
        ...
        logger.info('设备:{},地址{}。有{}条规则,MODBUS地址HOLDING_REGISTERS长度{},MODBUS地址COILS长度{}'.format(\
                i,config['scrept'][i][0]['adr'],len(config['scrept'][i])-3\
...
def getdata(ts,listid):  #获取数据
        ...
        while 1:
            ...
            try:
                ...
                time.sleep(int(sr_config['server']['redis_delay']))
                #print(data)

def mb_set_float(slave,b_name,addr,val):   #写入flaot格式数据
    if abs(val)<0.001:val=0
    j=struct.unpack('>HH',struct.pack('>f',round(val,2)))
...
def runsc(dirv_i):
    ...
    while 1:
        if init_flag:  #如果redis连接正常执行
            ...
            time.sleep(int(sr_config['server']['modbus_delay']))
        else:
            log2.info('连接redis失败，等待redis连接成功，暂停modbus数据刷新')  #如果redis连接失败，30s后重试mosbus数据更新
            time.sleep(10)
        #time.sleep(int(sr_config['server']['modbus_delay']))  #正常运行没4秒更新modbus数据
...
if __name__=='__main__':
    ...
    configdict={'server':{'redis_host':'192.168.1.93','redis_port':'6379','redis_delay':'5','modbus_port':'502','modbus_delay':'5',
            'filename':'config.yaml'}
            }
    
    if path.exists("config.ini"):  #如果配置文件存在,导入配置文件
        sr_config.read("config.ini",encoding='utf-8')
    else:
        sr_config.read_dict(configdict)
        sr_config.write(open("config.ini","w"))
    #############################
    filename=sr_config['server']['filename']
    logger.info("-"*50)
    logger.info("作者:谭润芝")
    ...
    if path.exists(filename):
        runflag=loadconfig(filename)
    else:
        log2.info('配置文件不存在，5秒后退出')
        logger.info('配置文件不存在，5秒后退出')
    if runflag:
        logger.info('redis服务器:{}\nredis端口:{}'.format(sr_config['server']['redis_host'],sr_config['server']['redis_port']))
        logger.info('modbus端口:{}'.format(sr_config['server']['modbus_port']))
        log2.info('redis服务器:{}\nredis端口:{}'.format(sr_config['server']['redis_host'],sr_config['server']['redis_port']))
        log2.info('modbus端口:{}'.format(sr_config['server']['modbus_port']))
        l3=jxconfig()
        log2.info('导入配置文件成功')
        ...
        server = modbus_tcp.TcpServer(port=int(sr_config['server']['modbus_port']))
        ...
        ts=xb_redis(sr_config['server']['redis_host'],int(sr_config['server']['redis_port']))  #定义redis服务
